                ...
                if (task.isSuccessful()){
                    ...
                    postedAds.clear();
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            ...
                            if (!ad.isClaimed() && location != null && !ad.getDonatorID().equals(profileID) && DistanceUtil.getDistanceDouble(location.getLatitude(), ad.getLocation().getLatitude(), location.getLongitude(), ad.getLocation().getLongitude(), 0.0, 0.0 ) < meterLimitForAd){
                                availableAds.add(ad);
                            }
                            if (ad.getDonatorID().equals(profileID)){
                                postedAds.add(ad);
