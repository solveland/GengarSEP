...
public class AddressDatabaseHelper extends SQLiteAssetHelper {
    ...
    public Cursor Autocomplete(CharSequence s){
        ...
        Cursor c = getReadableDatabase().rawQuery("SELECT * FROM addresses WHERE street LIKE '" + text + "%' or (street like '" + addressPart + "%' and housenumber like '" + numberPart + "%') order by housenumber limit 5 ",null);
        return c;
